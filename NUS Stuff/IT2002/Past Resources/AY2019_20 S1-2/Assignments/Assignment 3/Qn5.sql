...
THEN (SELECT COUNT(R.giver)
FROM Recommends R
WHERE R.giver = C.uname)
ELSE (SELECT COUNT(R.receiver)
FROM Recommends R
WHERE R.receiver = C.uname)
END
AS num
FROM Customers C
WHERE (SELECT COUNT(R.giver)
FROM Recommends R
WHERE R.giver = C.uname) > 0
AND EXISTS
(SELECT uname
FROM PetOwner P
WHERE P.uname = C.uname)
OR (SELECT COUNT(R.receiver)
FROM Recommends R
WHERE R.receiver = C.uname) > 0
AND NOT EXISTS
(SELECT uname
FROM PetOwner P
WHERE P.uname = C.uname);
